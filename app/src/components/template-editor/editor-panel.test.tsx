...
vi.mock('@/components/email-editor', () => ({
  EmailEditor: ({ onChange, initialContent }: { 
    onChange?: (blocks: PartialBlock[], html: string, text: string, placeholders: string[], warnings: string[]) => void
    ...
    <div data-testid="mock-email-editor">
      ...
      <span data-testid="initial-content">{JSON.stringify(initialContent)}</span>
    </div>
  ),
...
describe('EditorPanel', () => {
  const defaultProps = {
    ...
    editorWarnings: [] as { code: string; message: string; severity: 'warning' | 'error' }[],
    onFormChange: vi.fn(),
  ...
  describe('Form Fields', () => {
    ...
    it('displays placeholder text for all inputs', () => {
      ...
      expect(screen.getByPlaceholderText(/e\.g\., welcome email v2/i)).toBeInTheDocument()
      expect(screen.getByPlaceholderText(/e\.g\., welcome to apme!/i)).toBeInTheDocument()
      expect(screen.getByPlaceholderText(/preview text that appears in email clients/i)).toBeInTheDocument()
    })
  })
  ...
  describe('Validation Warnings', () => {
    it('does not show warnings section when no warnings exist', () => {
      ...
      expect(screen.queryByText(/compatibility warnings/i)).not.toBeInTheDocument()
    })
    ...
    it('displays warnings when they exist', () => {
      const propsWithWarnings = {
        ...defaultProps,
        editorWarnings: [
          { code: 'WARN_1', message: 'Warning message 1', severity: 'warning' as const },
          { code: 'WARN_2', message: 'Warning message 2', severity: 'warning' as const }
        ]
      }
    ...
    it('renders warning list items correctly', () => {
      const propsWithWarnings = {
        ...defaultProps,
        editorWarnings: [
          { code: 'NORMALIZATION_1', message: 'Unsupported tag removed', severity: 'warning' as const }
        ]
      }
      render(<EditorPanel {...propsWithWarnings} />)
  ...
  describe('Detected Placeholders', () => {
    it('does not show placeholders section when none detected', () => {
      ...
      expect(screen.queryByText(/detected placeholders/i)).not.toBeInTheDocument()
    })
