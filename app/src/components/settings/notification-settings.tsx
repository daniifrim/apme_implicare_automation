...
import { Mail, Smartphone, Webhook, AlertTriangle } from "lucide-react";
import {
  Card,
  CardContent,
  CardDescription,
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Notify when a new form submission is received
              </p>
            </div>
            <Switch
              checked={localSettings.emailOnNewSubmission}
              onCheckedChange={(checked) =>
                handleChange({ emailOnNewSubmission: checked })
              }
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Notify when email assignment fails
              </p>
            </div>
            <Switch
              checked={localSettings.emailOnAssignmentFailure}
              onCheckedChange={(checked) =>
                handleChange({ emailOnAssignmentFailure: checked })
              }
              disabled={!localSettings.emailEnabled}
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Notify on user invites, logins, and role changes
              </p>
            </div>
            <Switch
              checked={localSettings.emailOnUserAction}
              onCheckedChange={(checked) =>
                handleChange({ emailOnUserAction: checked })
              }
              disabled={!localSettings.emailEnabled}
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(value) =>
                handleChange({
                  emailDigest: value as typeof localSettings.emailDigest,
                })
              }
              disabled={!localSettings.emailEnabled}
            >
        ...
        <CardContent>
          <div className="flex items-center justify-between">
            <div className="space-y-0.5">
              ...
              <p className="text-sm text-muted-foreground">
                Forward events to configured webhook URLs
              </p>
            </div>
            <Switch
              checked={localSettings.webhookEnabled}
              onCheckedChange={(checked) =>
                handleChange({ webhookEnabled: checked })
              }
            />
          </div>
        </CardContent>
      ...
      <Card className="border-yellow-200 bg-yellow-50/50">
        ...
        <CardContent>
          <p className="text-sm text-yellow-700">
            Free tier: Up to 100 emails/day. Upgrade to Pro for unlimited
            notifications.
          </p>
        </CardContent>
      </Card>
