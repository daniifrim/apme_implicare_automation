...
export async function GET(request: NextRequest) {
  try {
    ...
    // Calculate metrics
    const metricsWhere: Prisma.WebhookEventWhereInput = dateFilter
    ...
    const successRate =
      totalEvents > 0 ? (successCount / totalEvents) * 100 : 0;
    ...
    if (completedEvents.length > 0) {
      const totalLatency = completedEvents.reduce((sum, event) => {
        if (event.processedAt && event.createdAt) {
          return (
            sum + (event.processedAt.getTime() - event.createdAt.getTime())
          );
        }
        return sum;
      }, 0);
