...
describe("GET /api/mappings", () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(prisma.fieldMapping.findMany).mockResolvedValue(
  ...
  it("should handle database errors", async () => {
    vi.mocked(prisma.fieldMapping.findMany).mockRejectedValue(
      new Error("DB error"),
    );
...
describe("POST /api/mappings", () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(prisma.filloutQuestion.findUnique).mockResolvedValue(
      mockQuestion as never,
    );
  });
  ...
  it("should create a new mapping when canonicalKey does not exist", async () => {
    ...
    vi.mocked(prisma.fieldMapping.create).mockResolvedValue(
      mockMapping as never,
    );
  ...
  it("should update existing mapping when canonicalKey exists", async () => {
    ...
    vi.mocked(prisma.fieldMapping.findUnique).mockResolvedValue(
      existingMapping as never,
    );
    vi.mocked(prisma.fieldMapping.update).mockResolvedValue(
      mockMapping as never,
    );
  ...
  it("should handle database errors", async () => {
    vi.mocked(prisma.fieldMapping.findUnique).mockRejectedValue(
      new Error("DB error"),
    );
