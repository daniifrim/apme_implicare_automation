...
describe("GET /api/dashboard/submissions/recent", () => {
  ...
  it("returns recent submissions with pagination", async () => {
    ...
    vi.mocked(prisma.submission.findMany).mockResolvedValueOnce(
      mockSubmissions as unknown as Awaited<
    ...
    const request = new Request(
      "http://localhost/api/dashboard/submissions/recent?page=1&limit=10",
    );
  ...
  it("respects custom pagination params", async () => {
    ...
    const request = new Request(
      "http://localhost/api/dashboard/submissions/recent?page=2&limit=5",
    );
    const response = await GET(request);
    const data = await response.json();
  ...
  it("limits max results to 50", async () => {
    ...
    const request = new Request(
      "http://localhost/api/dashboard/submissions/recent?limit=100",
    );
    await GET(request);
  ...
  it("handles database errors gracefully", async () => {
    ...
    const request = new Request(
      "http://localhost/api/dashboard/submissions/recent",
    );
    const response = await GET(request);
    const data = await response.json();
