...
describe('Assignment Management API', () => {
  ...
  describe('PATCH /api/assignments/[id]', () => {
    it('should update assignment status', async () => {
      ...
      vi.mocked(prisma.assignment.update).mockResolvedValue(mockAssignment as never)

      const request = {
        json: () => Promise.resolve({ status: 'sent' })
      } as unknown as Request

      const response = await PATCH(request, { params: Promise.resolve({ id: 'assign-1' }) })
    ...
    it('should accept all valid statuses', async () => {
      ...
      for (const status of validStatuses) {
        vi.mocked(prisma.assignment.update).mockResolvedValue({ id: 'assign-1', status } as never)
  ...
  describe('DELETE /api/assignments/[id]', () => {
    it('should delete assignment successfully', async () => {
      vi.mocked(prisma.assignment.delete).mockResolvedValue({ id: 'assign-1' } as never)

      const request = {} as Request
      const response = await DELETE(request, { params: Promise.resolve({ id: 'assign-1' }) })
      const body = await response.json()
    ...
    it('should return 404 if assignment not found', async () => {
      vi.mocked(prisma.assignment.delete).mockRejectedValue(new Error('Record not found'))
    ...
    it('should handle database errors', async () => {
      vi.mocked(prisma.assignment.delete).mockRejectedValue(new Error('DB error'))
