...
describe('POST /api/submissions/bulk/reprocess', () => {
  ...
  it('should handle partial failures', async () => {
    vi.mocked(reprocessAssignments)
      .mockResolvedValueOnce({ created: 2, skipped: 0, errors: [] })
      .mockResolvedValueOnce({ created: 0, skipped: 0, errors: ['Template not found'] })
  ...
  it('should handle complete failure', async () => {
    vi.mocked(reprocessAssignments).mockRejectedValue(new Error('Database error'))
  ...
  it('should handle single submission', async () => {
    vi.mocked(reprocessAssignments).mockResolvedValue({ created: 3, skipped: 0, errors: [] })
