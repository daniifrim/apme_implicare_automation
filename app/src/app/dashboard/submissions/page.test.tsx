  ...
  beforeEach(() => {
    vi.spyOn(global, 'fetch').mockImplementation((input) => {
      ...
      if (url.startsWith('/api/submissions?')) {
        return Promise.resolve(createFetchResponse(submissionsListResponse)) as Promise<Response>
      }
      if (url === '/api/submissions/sub-1') {
        return Promise.resolve(createFetchResponse(submissionDetailResponse)) as Promise<Response>
      }
