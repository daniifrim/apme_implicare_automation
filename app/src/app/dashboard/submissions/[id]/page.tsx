...
export default function SubmissionDetailPage({ params }: { params: { id: string } }) {
  ...
  async function handleAddAssignment() {
    ...
    try {
      const response = await fetch(`/api/submissions/${submission.id}/assignments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
              ...
              {submission.assignments.map((a) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      value={a.status}
                      onChange={(e) => handleUpdateAssignmentStatus(a.id, e.target.value)}
                      className={cn(
                        ...
                        a.status === 'cancelled' && 'bg-gray-50 border-gray-200'
                      )}
                    >
                      <option value="pending">Pending</option>
              ...
              {submission.answers.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-accent/30">
                    <td className="px-4 py-3 text-sm font-medium">{a.question.name}</td>
