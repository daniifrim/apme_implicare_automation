...
const statusConfig: Record<string, { icon: typeof CheckCircle; color: string; bg: string; label: string }> = {
  completed: { icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-100', label: 'Accepted' },
...
export default function WebhooksPage() {
  ...
  const [metrics, setMetrics] = useState<Metrics>({ total: 0, successRate: 0, avgLatency: 0 })
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-card border rounded-custom p-4">
          ...
          <div className="text-2xl font-bold mt-1">{metrics.total.toLocaleString()}</div>
        </div>
        <div className="bg-card border rounded-custom p-4">
          <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
            Success Rate
          </div>
          <div className={`text-2xl font-bold mt-1 ${metrics.successRate >= 95 ? 'text-green-600' : metrics.successRate >= 80 ? 'text-amber-600' : 'text-red-600'}`}>
        ...
        <div className="bg-card border rounded-custom p-4">
          ...
          <div className="text-2xl font-bold mt-1">{formatLatency(metrics.avgLatency)}</div>
        </div>
      </div>
      ...
      <div className="bg-card border rounded-custom overflow-hidden">
        <div className="divide-y">
          {loading && events.length === 0 ? (
            ...
            <div className="p-8 text-center text-muted-foreground">No webhook events found</div>
          ) : (
            events.map((event) => {
              const config = statusConfig[event.status] || statusConfig.pending
              const StatusIcon = config.icon
              const latency = event.processedAt && event.createdAt
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-mono text-xs font-bold">{event.eventId}</span>
                      <span className="text-xs text-muted-foreground">
                    ...
                    <div className="text-sm text-muted-foreground">
                      {event.status === 'failed' ? (
                        <span className="text-red-600">{event.errorMessage || 'Processing failed'}</span>
                      ) : (
                        <>
                          {config.label} via{' '}
                          <code className="text-xs bg-muted px-1 rounded">{event.eventType}</code>
                        </>
                      )}
                    </div>
                  </div>
