...
describe("assignments", () => {
  ...
  function createNormalizedSubmission(
    ...
    return {
      ...
      church: "Test Church",
      rawData: {},
      answers: [{ questionId: "q1", value: "Yes", rawValue: "Yes" }],
      ...overrides,
    };
  }
   ...
   */
  function createMatchingAnswers(
    type: string,
    value: string,
    ...
    return [
      {
        questionId: questionIdMap[type] || "generic_question",
        value,
        rawValue: value,
      },
    ];
  ...
  describe("markSubmissionAsProcessed", () => {
    ...
    it("should throw error if submission not found", async () => {
      vi.mocked(prisma.submission.update).mockRejectedValue(
        new Error("Record not found"),
      );

      await expect(markSubmissionAsProcessed("nonexistent")).rejects.toThrow(
        "Record not found",
      );
    });
  });
