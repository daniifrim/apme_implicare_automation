...
export function extractPlaceholders(content: string): string[] {
  ...
  return matches
    ? [...new Set(matches.map((m) => m.slice(2, -2).trim()).filter(Boolean))]
    : [];
}
...
function normalizeAnchors(inputHtml: string, warnings: string[]): string {
  return inputHtml.replace(/<a\b([^>]*)>/gi, (_match, attrs: string) => {
    ...
    if (!sanitizedHref) {
      warnings.push(
        "Removed an unsupported link URL. Allowed protocols: http, https, mailto, tel.",
      );
      return "<a>";
    }
...
export function normalizeEmailHtml(inputHtml: string): NormalizedEmailContent {
  ...
  if (/(class=|style=|font-family|mso-|data-)/i.test(html)) {
    warnings.push(
      "Removed unsupported rich formatting attributes from pasted content.",
    );
  }
  ...
  html = html
    .replace(
      /<\/?(?:div|section|article|header|footer|main|aside|h[1-6]|blockquote|pre)\b[^>]*>/gi,
      () => {
    ...
    .replace(/<\/?(?:table|thead|tbody|tr|td|th|span)\b[^>]*>/gi, () => {
      warnings.push(
        "Removed unsupported table/span markup for email compatibility.",
      );
      return "";
    });
...
export function collectTemplatePlaceholders(
  ...
  return [
    ...new Set(fields.flatMap((field) => extractPlaceholders(field ?? ""))),
  ];
}
