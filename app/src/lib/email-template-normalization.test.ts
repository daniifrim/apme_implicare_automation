...
describe('email template normalization', () => {
  it('should strip unsupported tags and attributes while preserving core content', () => {
    const input = '<div class="x"><p style="color:red">Hello <span>{{FirstName}}</span></p><script>alert(1)</script></div>'
  ...
  it('should keep valid links and remove unsafe protocols', () => {
    const input = '<p><a href="https://apme.ro" style="font-weight:700">safe</a> <a href="javascript:alert(1)">bad</a></p>'

    const result = normalizeEmailHtml(input)

    expect(result.html).toContain('<a href="https://apme.ro">safe</a>')
    expect(result.html).toContain('<a>bad</a>')
    expect(result.warnings.some((warning) => warning.includes('unsupported link URL'))).toBe(true)
  })

  it('should generate stable text output from normalized HTML', () => {
    const text = htmlToEmailText('<p>Buna {{FirstName}}</p><ul><li>Item 1</li><li>Item 2</li></ul>')
  ...
  it('should detect placeholders from all template fields', () => {
    ...
    expect(extractPlaceholders('{{FirstName}} {{LastName}} {{FirstName}}')).toEqual(['FirstName', 'LastName'])
  })
})
