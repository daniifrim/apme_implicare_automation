...
describe('AssignmentEngine', () => {
  ...
  function createSubmission(answers: Record<string, unknown> = {}): NormalizedSubmission {
    return {
      id: 'test-id',
  ...
  it('should assign prayer for ethnic group template', () => {
    ...
    expect(results.some(r => r.templateSlug === 'rugaciune-grup-etnic')).toBe(true)
  })
  ...
  it('should assign prayer for missionaries template', () => {
    ...
    expect(results.some(r => r.templateSlug === 'rugaciune-misionari')).toBe(true)
  })
  ...
  it('should assign short-term missions template', () => {
    ...
    expect(results.some(r => r.templateSlug === 'info-misiune-termen-scurt')).toBe(true)
  })
  ...
  it('should assign volunteer template', () => {
    ...
    expect(results.some(r => r.templateSlug === 'info-voluntariat')).toBe(true)
  })
  ...
  it('should assign Kairos course template', () => {
    ...
    expect(results.some(r => r.templateSlug === 'info-curs-kairos')).toBe(true)
  })
  ...
  it('should assign multiple templates based on multiple interests', () => {
    ...
    expect(results.some(r => r.templateSlug === 'info-misiune-termen-scurt')).toBe(true)
    expect(results.some(r => r.templateSlug === 'info-voluntariat')).toBe(true)
    expect(results.some(r => r.templateSlug === 'info-tabere-misiune')).toBe(true)
    expect(results.some(r => r.templateSlug === 'info-curs-kairos')).toBe(true)
