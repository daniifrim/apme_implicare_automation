 ...
 */
function convertAnswersToRecord(
...
export async function createAssignmentsForSubmission(
  ...
  try {
    ...
    const templateMap = new Map<string, Template>(
      templates.map((t: { slug: string; [key: string]: unknown }) => [
        t.slug,
        t as Template,
      ]),
    );
    ...
    for (const assignment of assignments) {
      try {
        ...
        result.errors.push(
          `Failed to create assignment for ${assignment.templateSlug}: ${errorMessage}`,
        );
      }
    }
      ...
      try {
        ...
        await prisma.assignment.create({
          data: {
            ...
            status: "pending",
            reasonCodes: [
              `Location-specific: ${normalizedSubmission.locationType}`,
            ],
          },
        });
        ...
        const errorMessage =
          error instanceof Error ? error.message : "Unknown error";
        result.errors.push(
          `Failed to create assignment for ${templateSlug}: ${errorMessage}`,
        );
      }
    }
 ...
 */
export async function markSubmissionAsProcessed(
  submissionId: string,
): Promise<void> {
 ...
 */
export async function reprocessAssignments(
  submissionId: string,
): Promise<AssignmentCreationResult> {
  ...
  const normalizedSubmission: NormalizedSubmission = {
    ...
    answers: submission.answers.map(
      (a: { questionId: string; value: string | null; rawValue: unknown }) => ({
