...
const COURSE_SLUG_MAP = {
  ...
  'Împuternicit pentru a influența': 'info-despre-cursul-imputernicit-pentru-a-influenta',
...
const SLUG_TO_NAME = {
  ...
  'info-despre-cursul-imputernicit-pentru-a-influenta': 'Info despre cursul Împuternicit pentru a influența',
  'info-despre-cursul-de-coordonatori-kairos': 'Info despre cursul de coordonatori Kairos',
...
function inferTemplatesFromRawData(rawData) {
  ...
  const missionField = t(r['Vrei să fii informat(ă) despre oportunitățile de a merge pe câmpul de misiune?'])
  ...
  // Prayer adoption
  const prayerAdoption = t(r['Vrei să adopți în rugăciune un misionar sau un popor neatins cu Evanghelia ?'])
  if (prayerAdoption === 'Misionar') {
    ...
    const ethnicGroupSelection = t(r['Pentru care popor neatins vrei să te rogi?'])
  ...
  // Camp info
  const campInfo = t(r['Vrei să primești informații despre taberele de misiune APME?'])
  ...
  // Course interest
  const courseInterest = t(r['Ești interesat(ă) să participi la anumite cursuri de pregătire când vor fi disponibile în zona ta?'])
