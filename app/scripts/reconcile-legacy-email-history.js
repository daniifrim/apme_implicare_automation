...
} = require('./legacy-inference-rules')
...
async function runPhase1EmailHistoryReconciliation(prisma, args, report) {
  ...
  if (unresolvedTemplates.size > 0) {
    console.log(`  ⚠️ Unresolved templates (skipped): ${unresolvedTemplates.size}`)
...
async function runPhase2InferenceReconciliation(prisma, args, report) {
  ...
  console.log(`  Submissions without assignments: ${submissionsWithoutAssignments.length}`)
  ...
  console.log(`  Submissions with zero matches: ${zeroMatchSubmissions.length}`)
...
async function main() {
  ...
  const defaultCsvPath = path.join(repoRoot, 'docs', 'data', 'email-history.csv')
  ...
  try {
    ...
    // Phase 1: Email History Reconciliation
    if (!args.skipPhase1) {
      const phase1Success = await runPhase1EmailHistoryReconciliation(prisma, args, report)
      if (!phase1Success) {
        process.exitCode = 1
        return
      }
    ...
    // Phase 2: Inference from Raw Data
    if (!args.skipPhase2) {
      const phase2Success = await runPhase2InferenceReconciliation(prisma, args, report)
      if (!phase2Success) {
        process.exitCode = 1
        return
      }
    ...
    if (report.phase1) {
      ...
      console.log(`  - Pairs from email history: ${report.phase1.desiredPairs}`)
      ...
      if (report.phase1.unresolvedTemplates.length > 0) {
        console.log(`  - Unresolved templates: ${report.phase1.unresolvedTemplates.length}`)
      }
    }

    if (report.phase2) {
      console.log(`\nPhase 2 (Inference):`)
      console.log(`  - Submissions checked: ${report.phase2.submissionsChecked}`)
      console.log(`  - Inferred assignments: ${report.phase2.inferredAssignments}`)
      console.log(`  - Zero-match submissions: ${report.phase2.zeroMatchSubmissions}`)
      if (!args.dryRun) {
