...
describe("legacy-email-history-utils", () => {
  describe("normalizeTemplateName", () => {
    it("should normalize diacritics, punctuation, and spacing", () => {
      expect(normalizeTemplateName("  Rugăciune pentru grup etnic  ")).toBe(
      ...
      ).toBe("info despre cursul imputernicit pentru a influenta");
      expect(normalizeTemplateName("Info  Tabere  Misiune   APME")).toBe(
  ...
  describe("getTemplateLookupKeys", () => {
    it("should generate candidates for Info prefix and Both prefix", () => {
      ...
      expect(
        getTemplateLookupKeys("Both - Rugăciune pentru grup etnic"),
      ).toEqual(
        expect.arrayContaining([
          "rugaciune pentru grup etnic",
      ...
      expect(getTemplateLookupKeys("Info Donații APME")).toEqual(
        expect.arrayContaining(["info donatii apme", "donatii apme"]),
      );
    });
  });
