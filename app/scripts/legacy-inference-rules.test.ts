...
describe('legacy-inference-rules', () => {
  describe('inferTemplatesFromRawData', () => {
    ...
    it('should skip mission template when user is excluded', () => {
      const rawData = {
        'Vrei să fii informat(ă) despre oportunitățile de a merge pe câmpul de misiune?': 'Nu acum, poate mai târziu'
      }
      const result = inferTemplatesFromRawData(rawData)
    ...
    it('should skip camp template for past participants', () => {
      const rawData = {
        'Vrei să primești informații despre taberele de misiune APME?': 'Am participat, doresc să mai fiu informat și pe viitor'
      }
      const result = inferTemplatesFromRawData(rawData)
      expect(result).toHaveLength(0)
    })
    ...
    it('should infer Kairos course template', () => {
      const rawData = {
        'Ești interesat(ă) să participi la anumite cursuri de pregătire când vor fi disponibile în zona ta?': 'Cursul Kairos'
      }
      const result = inferTemplatesFromRawData(rawData)
      expect(result).toHaveLength(1)
    ...
    it('should infer Mobilizează course template', () => {
      const rawData = {
        'Ești interesat(ă) să participi la anumite cursuri de pregătire când vor fi disponibile în zona ta?': 'Cursul Mobilizează'
      }
      const result = inferTemplatesFromRawData(rawData)
      expect(result).toHaveLength(1)
